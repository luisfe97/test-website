...
function AccountReg(props) {
  ...
  return (
    <RegCard className={props.className} cardClassName="w-full h-full rounded-none flex flex-col flex-wrap justify-center items-center">
      <Row className="text-xl font-bold py-4 text-gray-900">{t("Create MGL Account")}</Row>

      <Form form={form} name="basic" initialValues={{ remember: true }} autoComplete="off" onValuesChange={changeState} validateTrigger="onSubmit">
        <Form.Item
        ...
        >
          <Input placeholder={t("E-mail address")} prefix={<MailOutlined className="m-2" />} className=" rounded-lg  bg-gray-200 text-black " onChange={(e) => setEmail(e.target.value)} />
        </Form.Item>
        <Form.Item
          name="Verification"
          rules={[
        ...
        >
          <Input
            ...
            suffix={
              <a onClick={() => hasErrors(form.getFieldsError())} className="bg-gray-800 text-md  text-white  rounded-xl py-1 px-4">
                {t("Send")}
              </a>
            }
        ...
        >
          <Input.Password placeholder={t("enter password here")} prefix={<LockOutlined className="m-2" />} className="rounded-lg  bg-gray-200" onChange={(e) => setPassword(e.target.value)} />
        </Form.Item>
        ...
        >
          <Input.Password placeholder={t("confirm password")} prefix={<LockOutlined className="m-2" />} className="rounded-lg  bg-gray-200" />
        </Form.Item>

        <Form.Item name="inviteCode" rules={[{ max: 10 }]}>
          <Input placeholder={t("Invite Code(Optional)")} prefix={<TeamOutlined className="m-2" />} className="rounded-lg  bg-gray-200" onChange={(e) => setInviteCode(e.target.value)} />
        </Form.Item>
        ...
        <Form.Item className="mt-2">
          <button type="submit" onClick={register} className="w-full bg-gray-800 text-lg font-bold text-white  rounded-lg py-2">
            {t("Create Account")}
          </button>
        </Form.Item>
      </Form>
      <div className="text-center">
        ...
        <Link to="/login" onClick={props.stepLogIn} className="myAnchor text-md ml-2">
          {t("Log In")}
        </Link>
      </div>
    </RegCard>
