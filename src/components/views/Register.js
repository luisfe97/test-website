...
function Register(props) {
  ...
  return (
    <RegCard className={props.className} cardClassName="w-full h-full rounded-none flex flex-col flex-wrap justify-center items-center">
      <div className="text-2xl font-bold mt-8 mb-12 text-gray-900 text-center">{t("Log In")}</div>
      <Form name="basic" initialValues={{ remember: true }} autoComplete="off" form={form} validateTrigger="onBlur">
        <Form.Item
          name={["email"]}
          rules={[
        ...
        >
          <Input size="large" placeholder={t("E-mail address")} prefix={<MailOutlined className="m-2" />} className=" rounded-lg p-3 bg-gray-200 text-black " onChange={(e) => setEmail(e.target.value)} />
        </Form.Item>
        ...
        >
          <Input.Password size="large" placeholder={t("enter password here")} prefix={<LockOutlined className="m-2" />} className="rounded-lg p-3 bg-gray-200" onChange={(e) => setPassword(e.target.value)} />
        </Form.Item>
