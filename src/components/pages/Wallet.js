...
import WalletConnect from "../component/WalletComponents/ConnectionWallet";
...
function Wallet() {
  ...
  useEffect(() => {
    ...
    if (!publicKey) openNotification("Wallet Access failed.", "You are not allowed!", false, () => (window.location.href = "/walletMain"));
    return () => {
  ...
  return (
    <>
            ...
            <Row className="h-full">
              <Col xs={{ span: 24 }} md={{ span: 18 }} className="flex flex-col">
                <Row className="w-full">
                  <Col xs={{ span: 24 }} md={{ span: 10 }} className="bg-white border-l-8 border-gray-200 p-4">
                    <div className="text-xl myColor1 p-0">
                      <a onClick={reload}>{loading ? <ReactLoading className="inline-block mr-2" type="spinningBubbles" color="#000" height={20} width={20} /> : <AiOutlineCheck className="inline-block mr-2" size={20} />}</a>
                      {t("Balance")}
                    </div>
                  ...
                  <Col xs={{ span: 24 }} md={{ span: 14 }} className="bg-white border-l-8 border-gray-200 p-4 ">
                    ...
                    <Row className="mt-2  text-center">
                      <Col span={10} className="text-overflow">
                        <QRCode size={100} value={publicKey} className="inline mr-2" />
                      </Col>
                      <Col span={14}>
                ...
                <Row className="bg-white border-l-8 border-gray-200 border-t-8 flex-grow ">
                  {idx === 0 ? <WalletPortfolio tokensInfo={tokensInfo} network={network.url} getAssets={reload} /> : idx === 1 ? <WalletSend network={network} tokensInfo={tokensInfo} getAssets={reload} transactions={transations} setTransactions={setTransactions} setIdx={setIdx} setStopMode={setStopMode} /> : idx === 2 ? <WalletBuy /> : idx === 3 ? <WalletActivity /> : idx === 4 ? <WalletManageKeys network={network} /> : idx === 5 ? <WalletProfile /> : null}
                </Row>
              </Col>
              <Col xs={{ span: 24 }} md={{ span: 6 }} className="bg-white border-l-8 border-gray-200 myColor1">
                <Row className="border-b-8 border-gray-200 p-4 text-center">
                  <Col span={12} className="flex flex-col items-center">
                    {connection ? <AiTwotoneEnvironment size={20} className="text-green-500 inline mb-1 mr-1" /> : <AiTwotoneEnvironment size={20} className="text-red-500 inline mb-1 mr-1" />}
                ...
                <Row className="p-4 ">
                  ...
                  {transations.map((item, idx) => (
                    <Col span={24} className="text-overflow">
                      <a target="_blank" href={`${network.explorer}tx/${item}`} className="my-2 myColor1">
                        {item}
                      </a>
                    </Col>
                  ))}
      ...
      <Drawer className="myColor1 text-lg font-bold" headerStyle={{ color: "red" }} title="Change Network" placement="right" onClose={() => setVisible(false)} visible={visible} closeIcon={<AiFillCloseCircle size={20} />}>
        <a
          onClick={() => {
