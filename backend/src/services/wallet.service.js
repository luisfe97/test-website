...
class WalletService {
  ...
  static async addToken(rawData, userId) {
    ...
    if (rawData.network === "polygon-mainnet") {
      if (wallet.polygonmain_assets.includes(rawData.token)) return { response: false, message: "Already added.", data: null };
      ...
      if (wallet.polygontest_assets.includes(rawData.token)) return { response: false, message: "Already added.", data: null };
      ...
      if (wallet.bscmain_assets.includes(rawData.token)) return { response: false, message: "Already added.", data: null };
      ...
      if (wallet.bsctest_assets.includes(rawData.token)) return { response: false, message: "Already added.", data: null };
      assets = wallet.bscmain_assets + "," + rawData.token_address;
      symbols = wallet.bsc_tokensymbol + "," + rawData.token_symbol;
      result = await WalletModel.update({ bsc_tokensymbol: symbols, bsctest_assets: assets }, wallet.id);
    } else if (rawData.network === "ethereum-mainnet") {
      if (wallet.weth_assets.includes(rawData.token)) return { response: false, message: "Already added.", data: null };
      assets = wallet.weth_assets + "," + rawData.token_address;
      symbols = wallet.weth_tokensymbol + "," + rawData.token_symbol;
      result = await WalletModel.update({ weth_tokensymbol: symbols, weth_assets: assets }, wallet.id);
    }
    if (result === "") {
  ...
  static async getBalance(rawData, userId) {
    ...
    if (rawData.network === "polygon-mainnet") {
      ...
      let result = await new Wallet(rawData.network).getBalance(wallet.publickey, wallet.weth_tokensymbol, wallet.weth_assets);
      return { response: true, message: "Success", data: result };
    }
  }
  ...
  static async getPrice(rawData, userId) {
    ...
    if (rawData.network === "polygon-mainnet") {
      ...
      return { response: true, message: "Success", data: result };
    } else if (rawData.network === "ethereum-mainne") {
      let result = await new Wallet(rawData.network).getTokenPrice(wallet.weth_assets);
      return { response: true, message: "Success", data: result };
    }
  }
  ...
  static async getAssets(rawData, userId) {
    ...
    if (rawData.network === "polygon-mainnet") {
      ...
      console.log(wallet.weth_assets);
      let result = wallet.weth_assets.split(",");
      return { response: true, message: "Success", data: result };
    }
  }
